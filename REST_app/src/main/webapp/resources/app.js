...
function delete_car(id) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			window.alert("Lugu kustutatud, miks nii tegid?");
...
function add_car() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			window.alert("Uus lugu sisestatud, sa oled väga tubli!");
			console.log(JSON.stringify(data));
...
function save_car() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			console.log(JSON.stringify(data));
			// uuendame nimekirja vormil , yks on uuendatud
			get_cars();
		}
	});
...
function display_car(car) {
	var out_data = "";
	out_data = out_data
			+ "<table bgcolor=eeeeee><tr><td>Muutmine. Loo id: <b>" + car.id
			+ "</b></td></tr>";

	out_data = out_data
			+ "<tr><td>Esitaja:</td><td><input type=text name=make value='"
			+ car.make + "'></td></tr>";
	out_data = out_data
			+ "<tr><td>Lugu:</td><td><input type=text name=model value='"
			+ car.model + "'></td></tr>";
	out_data = out_data
			+ "<tr><td>Žanr:</td><td><input type=text name=series value='"
			+ car.series + "'></td></tr>";
	out_data = out_data
...
function display_cars(data) {
	var out_data = "";
	out_data = out_data
			+ "<table bgcolor=eeeeee><tr><td colspan=4>Lugusid kokku: <b>"
			+ data.length + "</b></td></tr>";
	for ( var i in data) {
		var car = data[i];
		out_data = out_data + "<tr><td>Esitaja:</td><td bgcolor=ffffff>"
				+ car.make + "</td><td>Lugu:</td><td bgcolor=ffffff>"
				+ car.model + "</td>";
		out_data = out_data
